...
通过源文件编译安装R需要在deepin系统中提前安装所需（依赖）的package。有两种方式解决依赖问题：
...
第二种方法：手动安装所依赖的package。这种手动安装依赖的package的方式或许可以节省电脑空间（没仔细比较过，不清楚）。通常，我们可以通过运行`configure`命令检查是否缺少依赖，当终端报错时，可根据提示安装相应的package。为了减少试错，这里列举出R依赖的package(适用于R3.6.3；可参考debian官网上对于R依赖的package的[介绍](https://packages.debian.org/buster/r-base-core))：
...
安装上依赖的package之后，就可以一路跑到底、编译成功。有些R package可能依赖其他的debian package，如果遇到这些问题，可自行寻找解决办法。下面将介绍编译安装R的步骤。
